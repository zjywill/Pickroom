#!/bin/bash
# Notarise a release that shipped without it, after Apple's notary queue comes
# back.
#
#   scripts/notarise-release.sh 0.3.0
#
...
# hours away. This is the other half: get tickets onto that build, replace the
# assets on the existing Release, and strip the workaround from the notes.
#
# **Run it repeatedly.** It does not wait for Apple — it advances as far as it
# can, remembers the submission id, and exits telling you to come back. Three
# runs, typically:
#
#   1st  submits the app
...
#
# A run that has nothing to wait for does the whole thing at once. Nothing is
# resubmitted, and a leg that is already done takes about a second.
#
# Both tickets. A ticket on the DMG covers the DMG and nothing else — drag the
# app to /Applications, throw the image away, and the ticket Gatekeeper reads
# on a Mac that is offline is the one inside the bundle. A read-only image
# cannot be stapled into after the fact, so the app comes out of the image,
# gets its own ticket, and the image is rebuilt around it. The binary is the
# one that shipped either way; only the tickets are new.
#
# No new version number. The tag, the release and the cask all still point at
# the same source.
#
# The notes edit is the reason this is a script. Re-uploading a notarised DMG
# under notes that still say "macOS blocks the first launch" leaves every
# reader following three steps they no longer need, and it is exactly the step
# a person doing this by hand forgets.
set -euo pipefail
...
# since a ticket is only useful next to the bytes it belongs to.
...
It picks up where this left off — no resubmission, and the leg that is already
done takes about a second. Watch the queue with:

  xcrun notarytool history --keychain-profile ${NOTARY_PROFILE:-<profile>}
EOF
}
...
# rebuild is a different binary, and pushing that out under a version that is
# already published replaces people's build with one nobody tested. This also
# makes the script work on a machine that never built the release.
...
# An ad-hoc signature cannot be notarised, and finding that out from Apple ten
# minutes later is a worse error message than this one.
#
...
# and `set -o pipefail` hands that 141 to the pipeline.
...
# which is exactly the trap on an account whose verdicts take hours and where
...
echo "==> Replacing the assets on $TAG"
write_checksum "$DMG"
gh release upload "$TAG" "$DMG" "$DMG.sha256" --repo "$REPO" --clobber

# The cask pins the DMG's sha256, and the file that was just uploaded is a
# different one — same version, same binary inside, but a rebuilt image with a
# ticket, so a different hash. Leaving the tap alone here would break
# `brew install --cask pickroom` with a checksum mismatch the moment the
# upload lands.
#
...
(
    ...
    if [ -z "$(git status --porcelain)" ]; then
        ...
        git add "$CASK"
        git commit -q -m "pickroom $VERSION: notarised DMG"
        git push -q origin HEAD
    fi
)

# The workaround block is matched on the sentence release.sh writes rather
# than on line numbers, so an edited changelog above it does not shift the
...
replacement = """**DMG** — no toolchain needed. Open it, drag Pickroom to Applications, launch
...
Check it the way a downloader would — this reads the ticket in the file rather
than asking Apple, which is the whole point:
