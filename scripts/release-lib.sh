...
#   NOTARY_PROFILE   notarytool keychain profile — see find_notary_profile
#   NOTARY_TIMEOUT   how long to wait for one verdict           (default 2h)
#   NOTARY_RETRIES   fresh submissions before giving up         (default 1)

# Wait long, resubmit rarely. Verdicts for this Developer ID arrive in hours,
# not the minutes the documentation leads you to expect: on its first day of
# notarising anything, one submission sat unanswered for eight hours while
# others were accepted two and a half hours in.
#
...
# staple it.
#
# Nothing is lost by giving up on the wait, either: the ticket keeps, and the
# staple-first path in notarise() picks it up on the next run in under a
...
# A notarytool profile holds Apple ID credentials for a team — it is not tied
# to one app, so any profile on this machine belonging to the same Developer
# ID can get a ticket for this build. Hence a candidate list rather than a
# literal: prefer one named after this app, otherwise use what the keychain
# already has instead of failing on a missing name.
NOTARY_PROFILE_CANDIDATES="${NOTARY_PROFILE_CANDIDATES:-pickroom thegit}"
...
# The first candidate profile the keychain actually answers for. One network
# round trip per candidate, so it is resolved lazily by notary_profile()
# rather than at source time — release.sh sources this file only to verify a
# finished DMG and never needs credentials at all.
find_notary_profile() {
    local candidate
...
# Resolves NOTARY_PROFILE once and caches it in the variable, so an explicit
# NOTARY_PROFILE=... in the environment always wins and the candidate search
# happens at most once per run.
notary_profile() {
    if [ -z "${NOTARY_PROFILE:-}" ]; then
        NOTARY_PROFILE="$(find_notary_profile || true)"
    fi
    [ -n "${NOTARY_PROFILE:-}" ] || return 1
    printf '%s\n' "$NOTARY_PROFILE"
}
...
notarise() { # <path to submit> <path to staple>
    ...
    notary_profile >/dev/null || return 1
    ...
    while :; do
        ...
        if grep -q "status: Accepted" "$log"; then
            rm -f "$log"
            # Checked, not fired and forgotten. At an `if ! notarise ...` call
            # site bash suspends errexit for the whole function, so an
        ...
        if [ "$attempt" -ge "$NOTARY_RETRIES" ]; then
            ...
            echo "    the ticket once it lands. Watch for it, then run this again:"
...
# adds one more item to a slow one, so a "check again" that blindly submitted
# would make things worse every time it was run.
#
...
# The DMG and its checksum move together: release.sh uploads both, and the
# cask pins the same hash. Writing it in one place keeps a rebuilt image from
# shipping under the hash of the one it replaced.
write_checksum() { # <dmg>
